...
(defproject dictionary-backend "0.0.1-SNAPSHOT"
  ...
  :profiles {:dev {:aliases {"run-dev" ["trampoline" "run" "-m" "dictionary-backend.server/run-dev"]}
                   :dependencies [[io.pedestal/pedestal.service-tools "0.5.11-beta-1"]]}
             :kaocha {:dependencies [[lambdaisland/kaocha "1.84.1335"]]}
             :uberjar {:aot [dictionary-backend.server]}}
  :aliases {"kaocha" ["with-profile" "+kaocha" "run" "-m" "kaocha.runner"]}
  :main ^{:skip-aot true} dictionary-backend.server)
